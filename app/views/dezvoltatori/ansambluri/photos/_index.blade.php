...
@section('datatable-specific-page-jquery-initializations')
	...
	var upload_document = $("#file-document").fileinput({
		...
		'uploadAsync'     : true,
		'uploadUrl'       : '{{URL::route('upload-ansamblu-photo', ['id_ansamblu' => $ansamblu->id])}}',
		'fileActionSettings' :
			{
	...
	$(document).on('click', '.action-delete-ansamblu-document', function(){
		$.ajax({
			'url'    : "{{URL::route('delete-ansamblu-photo')}}",
			'type'   : 'post',
			dataType : 'json',
