...
class CreateDezvoltatoriOfertaPdf
{
    protected $orientation  = 'P';
    protected $pagesize     = 'A4';
    protected $destionation = 'D';
    protected $dezvoltator   = NULL;
    protected $redus        = false;

    public function __construct($orientation, $pagesize, $destionation, $dezvoltator, $redus = false)
    {
        if( is_null($dezvoltator) )
        {
            throw new \Exception("Nu există dezvoltaotrul pentru care să se genereze oferta");
        }
    ...
    public function __call($method, $args)
    {
        ...
        if( array_key_exists($method, $attributes = $this->dezvoltator->getAttributes()) )
        {
            return $attributes[$method];
        }
        throw new \Exception("Dezvoltator nu are proprietatea [$method]");
    }

    protected function fileName()
    {
        $file_name = str_replace('\\', '/', storage_path() . '/app/dezvoltatori/oferte/oferte-dezvoltatori-' . $this->id()) . '.pdf';
    ...
    protected function outnume()
    {
        ...
        if( ! $nume )
        {
            ...
            $nume = 'Numele dezvoltatorului nu este definit';
        }
    ...
    protected function outphotos()
    {
        ...
        if($number_of_photos = $photos->count())
        {
            ...
            foreach($photos as $i => $photo)
            {
                ...
                $new_file = str_replace('\\', '/', storage_path() . '/app/dezvoltatori/photos/resized/' . $photo->id  . '.png');
