...
class CreateApartamentDezvoltatoriOfertaPdf
{
    ...
    public function __construct($orientation, $pagesize, $destionation, $apartament, $redus = false)
    {
        ...
        $this->cladire      = \Imobiliare\Cladire::getRecord($apartament->id_cladire);
        $this->redus        = $redus;
    ...
    {
        $this->pdf->Pdf()->SetFont('freeserif', '', 12, '', false);
        $this->pdf->Cell()->text($this->observati_cladire())->width(190)->halign('L')->background([255, 255, 255])->out()->reset('background');
        $this->pdf->Pdf()->ln();
    }

    public function outChapterBloc()
    {

//        $this->pdf->Pdf()->SetXY(10, 185);
        $this->pdf->Pdf()->SetFont('freeserif','B', 14,'', false);
        $this->pdf->Cell()->text('Detalii clădire')->width(190)->border('TB')->halign('C')->background([210, 210, 210])->out()->reset('background');
        $rows = [
    ...
    public function observati_cladire()
    {
    ...
    {
        $this->pdf->Pdf()->SetFont('freeserif', 'B', 14, '', false);
        $this->pdf->Cell()->text('Date generale apartamente')->width(190)->border('TB')->halign('C')->background([210, 210, 210])->out()->reset('background');
        $rows = [
